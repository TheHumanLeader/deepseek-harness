...
flowchart LR
  ...
  pkg_agent_guidance["agent-guidance"]
  svc_agentGuidance["ctx.agentGuidance<br/>Live main and helper Agent configuration"]
  ...
  pkg_agent_guidance --> svc_agentGuidance
  ...
  svc_agentGuidance --> pkg_tool_subagent
...
| `ctx.agentGuidance` | `core` | [`agent-guidance`](../packages/context/agent-guidance) | - | [`tool-subagent`](../packages/subagent/tool-subagent) | - | 在每次模型请求前读取最新的全局、项目和辅助 Agent 设置，并在受管理委派开始时解析辅助 Agent 的提示词、模型路线和工具权限。 |
