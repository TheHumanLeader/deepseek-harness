...
在不定义工具的情况下添加模型可见请求上下文的产品插件。`agent-instructions` 与 `agent-guidance` 默认启用；`time-context`、`tmux-context` 和 `session-reference` 需主动启用。
...
| [`agent-guidance/`](agent-guidance/README.md) | 用户配置的全局主 Agent 指令 | — |
