...
Product plugins that add model-visible request context without defining a tool. `agent-instructions` and `agent-guidance` are included by default; `time-context`, `tmux-context`, and `session-reference` are opt-in.
...
| [`agent-guidance/`](agent-guidance/README.md) | User-configured global main-Agent instructions | — |
