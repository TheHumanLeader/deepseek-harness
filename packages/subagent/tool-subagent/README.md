...
| `managedAgentSelector` | Adds a required model-facing `agent` id and resolves that Agent's latest prompt, model route, and tool allow-list from `agent-guidance` when the child starts. |
...
- **Static instances keep fixed policy** — when `managedAgentSelector` is off, another model, persona, tool filter, or depth cap requires another distinctly named tool.
