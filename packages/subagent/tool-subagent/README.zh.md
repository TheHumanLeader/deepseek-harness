...
| `managedAgentSelector` | 增加必填的模型参数 `agent`，并在子 Agent 启动时从 `agent-guidance` 读取该 Agent 最新的提示词、模型路由和工具白名单。 |
...
- **静态实例的策略固定**：关闭 `managedAgentSelector` 时，其他模型、persona、工具过滤器或深度上限需要另一个名称不同的工具。
