...
Read-only Host projection of the current Cordis Loader tree. `PluginInventoryGateway` registers the `pluginInventory` service and publishes `pluginInventory/list`. Every call reads `ctx.loader.entries()` directly and skips structural group rows. Each result includes Loader state plus purpose, features, read/change/send behavior, network and credential requirements, Agent access, an unavailable reason, and a package-documentation link when declared.
...
- **Point-in-time state only** — the result contains no durable failure history or subscription. Cordis does not expose a failed Fiber's private error, so a failed row directs users to the Host log instead of inventing a cause.
